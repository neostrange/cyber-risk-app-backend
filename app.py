...
def get_vulnerabilities():
    with driver.session() as session:
        ...
        result = session.run(query)
        vulnerabilities = [record["v"]._properties for record in result]
...
def create_vulnerability():
    ...
    if not all(key in data for key in ("vulnID", "vulnName", "severity")):
...
def get_dashboard_stats():
    with driver.session() as session:
        stats = {}
        # Count total assets
        stats['total_assets'] = session.run("MATCH (a:Asset) RETURN count(a) as count").single()['count']
        # Count total threats
        stats['total_threats'] = session.run("MATCH (t:Threat) RETURN count(t) as count").single()['count']
        # Count total vulnerabilities
        stats['total_vulnerabilities'] = session.run("MATCH (v:Vulnerability) RETURN count(v) as count").single()['count']
        # Count total incidents
        stats['total_incidents'] = session.run("MATCH (i:Incident) RETURN count(i) as count").single()['count']
        # Count high-risk assets
        stats['high_risk_assets'] = session.run("MATCH (a:Asset) WHERE a.riskScore > 3 RETURN count(a) as count").single()['count']
